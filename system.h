...
#define SYSDEF(name, ...)  \
	...
	void Update ## name ## System() { \
		update(components, numComponents); \
	} \
	void Add ## name(Entity e, __VA_ARGS__) { \
		{ \
		struct entityToComponent *c; \
...
#define SYSFUNC(name, method, ...) \
		name##method(Entity e, __VA_ARGS__) { \
			struct name *C = Get ## name(e);

/* SYSCALL attempts to invoke method on the entity that C belongs to. */
#define SYSCALL(sys, method, C, ...) do { \
