...
static void key(Entity e, int key, int button, int action, int mods) {
	UNUSED(button);
	UNUSED(mods);
...
void AddConsole(Entity e) {
	...
	InputRegisterKeyEvent(e, INPUT_LAYER_CONSOLE, key);
...
void InitConsoleSystem(GLFWwindow *window) { win = window; }
